...
foreach ($perfresourcelist1 in $perfresourcelist){
    ...
    $asptiername = (Get-AzAppServicePlan -ResourceGroupName $perfresourcelist1.ResourceGroupName -Name $aspname).Sku.Tier
        ...
        if ($asptiername -match "Standard"){

        Set-AzAppServicePlan -Tier PremiumV2 -Name $aspname -ResourceGroupName $perfresourcelist1.ResourceGroupName -Verbose
        $newasptier = (Get-AzAppServicePlan -ResourceGroupName $perfresourcelist1.ResourceGroupName -Name $aspname).Sku.Tier
        Write-Host "App Service Plan" $aspname "is Upgraded to" $newasptier -BackgroundColor DarkGreen
    ...
    $conregistrysku = $conregistryname.SkuTier
        ...
        if ($conregistrysku -match "Standard"){
        
        Update-AzContainerRegistry -ResourceGroupName $perfresourcelist1.ResourceGroupName -Name $perfresourcelist1.Name -Sku Premium -Verbose
        $newconregistrysku = (Get-AzContainerRegistry -ResourceGroupName $perfresourcelist1.ResourceGroupName).Name
        Write-Host "Container Registry" $conregistryname "is Upgraded to" $newconregistrysku -BackgroundColor DarkGreen
    ...
    $databasename = (Get-AzSqlDatabase -ResourceGroupName $perfresourcelist1.ResourceGroupName -ServerName $sqlserver)
            ...
            $databasename2 = Get-AzSqlDatabase -ResourceGroupName $perfresourcelist1.ResourceGroupName -ServerName $sqlserver -DatabaseName $databasename1
                ...
                if ($databasename2.SkuName -match "Basic"){
                Set-AzSqlDatabase -DatabaseName $databasename2.DatabaseName -ServerName $sqlserver -ResourceGroupName $perfresourcelist1.ResourceGroupName -Edition Standard -Verbose
                ...
                Write-Host "SQL Database" $databasename2.DatabaseName "is Upgraded to" $newdbtier -BackgroundColor DarkGreen
